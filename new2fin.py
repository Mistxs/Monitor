...
logging.basicConfig(filename='840648.txt', level=logging.INFO, format='%(asctime)s - %(message)s')
...
def job():
    ...
    for i in range(len(staff)):
        ...
        if len(dates) > 0:
            message = f"Филиал {salon_id} (РОО Теннисный клуб) Есть сломанные дни у сотрудника {staff[i]}: {dates}"
