...
def getshedule(staff):
    url = f"https://api.yclients.com/api/v1/schedule/{salon_id}/{staff}/2023-06-07/2023-11-11"
    payload = {}
    response = requests.request("GET", url, headers=headers, data=payload).json()
    print(response["data"])
...
def repairslots(staff):
        response = getshedule(staff)
...
def job():
    ...
    for i in range(len(staff)):
        ...
        while current_date <= end:
            repairslots(staff[i], current_date.strftime("%Y-%m-%d"))
        ...
        if len(dates) > 0:
            message = f"Филиал {salon_id} (PianoRooms) Есть сломанные дни у сотрудника {staff[i]}: {dates}"
